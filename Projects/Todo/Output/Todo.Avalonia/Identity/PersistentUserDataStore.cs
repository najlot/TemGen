...
namespace Todo.Avalonia.Identity;
...
public class PersistentUserDataStore : IUserDataStore
{
	...
	private readonly string _storagePath = BuildStoragePath();
	...
	private async Task SaveUserData(UserData data)
	{
		...
		try
		{
			...
			{
				payload = ProtectForWindows(payload);
			}
	...
	private static string BuildStoragePath()
	{
		...
		return Path.Combine(basePath, "Todo", "Identity", "userData.dat");
	}
