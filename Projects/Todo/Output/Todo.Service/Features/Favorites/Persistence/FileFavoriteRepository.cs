...
public sealed class FileFavoriteRepository : IFavoriteRepository
{
	private readonly string _storagePath;

	public FileFavoriteRepository(FileConfiguration configuration)
	{
		_storagePath = configuration.FavoritesPath;
	...
	public IQueryable<FavoriteModel> GetAllQueryable()
	{
		...
		foreach (var path in Directory.GetFiles(_storagePath))
		{
			...
			var item = JsonSerializer.Deserialize<FavoriteModel>(bytes, ServiceJsonSerializer.Options);
			if (item is not null)
			{
				items.Add(item);
			}
	...
	public async Task<FavoriteModel?> Get(Guid id)
	{
		...
		return JsonSerializer.Deserialize<FavoriteModel>(bytes, ServiceJsonSerializer.Options);
	}
	...
	public async Task Update(FavoriteModel model)
	{
